  ...
  for await (const message of streamClient.streamData({
    ...
    switch (message._tag) {
      ...
      case "data": {
        ...
        for (const block of message.data.data) {
          ...
          for (const event of block.events) {
            const eventKey: EventKey = {
              ...
              transactionHash: event.transactionHash, // store as hex string for full precision
            };
        ...
        // TEMPORARILY DISABLED: Materialized view refresh while catching up
        // Re-enable once caught up to current blocks
